...
class NbiExtract extends Command
{
     ...
     * @var string
     */
    protected $signature = 'Nbi:backup';
    ...
    {   
        ...
        foreach ($result as $key => $values) {
         $B_NBIOT = new LteBackup("B_NBIOT","city","hour",$values->connName,"NBIOT",$db);
         $citys = Databaseconns::select()->where('connName','like',$values->connName."%")->get()->toArray();
            ...
            NBIOT::where(['location'=>$values->connName,'day_id'=>$startTime,'hour_id'=>$hour])->update(['interfererate'=>$num]);
