...
class LteQuery{
	...
	public static function queryTemplate($db,$pmDB,$subNets,$counters,$template,$locationDim,$timeDim,$city,$type,$interval){
		    if ($timeDim == "hour") {
	...
	public static function createSql($localDB,$pmDB,$kpiName,$subNetwork,$counters,$template,$locationDim,$timeDim,$city,$type,$startTime,$endTime)
	{
	    ...
	    $citySQL = "substring(SN,charindex('=',substring(SN,32,25))+32,charindex(',',substring(SN,32,25))-charindex('=',substring(SN,32,25))-1)";
	    ...
	    if(count($tables) == 1 && $templateName != 'badHandoverCell') {
	        $currTable = $tables[0];
