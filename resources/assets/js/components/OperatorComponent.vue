<template>
    <div>
        ...
        <v-tabs v-if="this.$store.getters.bOperatorsStatus===2" flat fixed-tabs slider-color="blue" v-model="active">
            <v-tab v-for="operator in operators" :key="operator.id" @click="operatorFix(operator.operator)" v-bind:disabled="operator.operator != 'mobile'">
                {{ operator.name }}
            </v-tab>
        </v-tabs>
    </div>
